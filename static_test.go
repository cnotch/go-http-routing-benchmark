...
func init() {
	...
	calcMem("ApiRouter", func() {
		staticApiRouter = loadApiRouter(staticRoutes)
	})
	calcMem("Bear", func() {
...
func BenchmarkApiRouter_StaticAll(b *testing.B) {
	benchRoutes(b, staticApiRouter, staticRoutes)
}
